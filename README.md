...
Although behavior is greatly improved, however, the swiss table does have a few limitations including the prerequisite that the hash function which generates the 64- bit value strategically “distributes entropy” across the bits<sup>2</sup>. Moreover, the implementation is not as widely adapted compared to conventional sets and maps.
